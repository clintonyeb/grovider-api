...
async function updateUser(request) {
  ...
  const userQuery = 'SELECT first_name,last_name,email,phone_number,avatar FROM users WHERE identity=$1 LIMIT 1'
  ...
  const query = 'UPDATE users SET first_name = $1, last_name = $2, email = $3, phone_number = $4, avatar = $5) WHERE identity=$6 RETURNING *;'
...
async function getUserWithEmail(request, email) {
  const db = request.server.app.pg
  const query = "SELECT id,first_name,last_name,email,password,identifier FROM users WHERE email = $1 LIMIT 1"
  const values = [email]
  const res = await db.query(query, values)
  return res.rows[0]
}
